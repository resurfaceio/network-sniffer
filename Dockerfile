...
RUN git clone http://github.com/resurfaceio/goreplay && cd goreplay && git checkout resurface
WORKDIR /go/goreplay
ARG TARGETOS TARGETARCH
RUN GOOS=$TARGETOS GOARCH=$TARGETARCH go build -o /out/gor -ldflags "-extldflags=-static -X main.VERSION=1.3.3-Resurface" .
